...
| 파일 | 성격 | 채워지는 경로 |
|---|---|---|
| `rrb_national_stats.json` | 전국 총계 시계열 | `scripts/fetch_rrb_api.py`(오픈API) 또는 `scripts/ingest_rrb_csv.py`(CSV) |
...
| `ingestion.tool` | 실제로 적재한 스크립트 경로 |
| `ingestion.sourceFile` / `sourceApiPath` | 적재 출처(CSV 파일명 또는 API 경로) |
